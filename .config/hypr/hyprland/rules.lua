...
--hl.animation({ leaf = "layersIn",  enabled = true, speed = 2.5, bezier = "emphasizedDecel", style = "slide down" })
--hl.animation({ leaf = "layersOut", enabled = true, speed = 2.2, bezier = "emphasizedAccel", style = "slide up" })

-- apps opacity
hl.window_rule({ match = { class = "^(app.zen_browser.zen)$" }, opacity = 0.9 })
hl.window_rule({ match = { class = "^(steam|vesktop|code-oss|org.kde.dolphin|kute)$" }, opacity = 0.8 })

-- floating for dialogues
...
-- PiP
hl.window_rule({ match = { title = "^([Pp]icture[-\\s]?[Ii]n[-\\s]?[Pp]icture)(.*)$" }, float = true, keep_aspect_ratio = true, pin = true })
hl.window_rule({ match = { title = "^([Pp]icture[-\\s]?[Ii]n[-\\s]?[Pp]icture)(.*)$" }, move = {"(monitor_w*0.73)", "(monitor_h*0.72)"}, size = {"(monitor_w*0.25)", "(monitor_h*0.25)"} })
...
hl.layer_rule({ match = { namespace = "quickshell:overview" }, animation = "slide top"})
